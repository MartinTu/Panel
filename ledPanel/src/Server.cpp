...
Server::Server(serverType_t type, unsigned int port) :
        stop(false), alive(true), mySocket(0), shut_down(false), listenSocket(0), type(type), port(port), clientAddrLen(
                0), close_connection(false)
{
    int rt;
    ...
    if (0 != rt)
    {
        throw MyException("Server: thread attribute initialization failed: " + string(strerror(rt)));
    }

    rt = pthread_attr_setdetachstate(&threadAttr, PTHREAD_CREATE_DETACHED);
    if (0 != rt)
    {
        throw MyException("Server: setting detach state failed: " + string(strerror(rt)));
    }
...
void Server::init()
{
    ...
    if (type == TCP)
    {
        ...
        {
            shutdown();
            throw MyException("unable to listen socket: " + string(strerror(errno)));
        }
    }
}
...
string Server::receiveCommand()
{
    ...
    {
        cerr << "[ERROR] receive failed: " << strerror(errno) << "(" << errno << ")" << endl;
    ...
    try
    {
        ...
        while (!shut_down)
        {
            ...
            if (type == TCP)
            {
                ...
                if (-1 == mySocket)
                {
                    cerr << "[ERROR] socket accept failed: " << strerror(errno) << "(" << errno << ")" << endl;
                    break;
                }
            } else
            {
            ...
            try
            {
                ...
                while ((!close_connection) && (!shut_down))
                {
                    ...
                    {
                        if ((0 != actClient.compare(inet_ntoa(clientAddr.sin_addr)))
                                || (actPort != clientAddr.sin_port))
                        {
                            actClient = inet_ntoa(clientAddr.sin_addr);
                            actPort = clientAddr.sin_port;
                            cout << "[INFO] connected with " << actClient << " on port 0x" << actPort << endl;
                        }
                    }
