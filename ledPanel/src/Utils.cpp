...
float Utils::sinf(float x)
{
	if(x < 0.0){
		...
		x -= floor(x);
	int xi = (int) round(x*SINSAMPLECOUNT);
	if ((xi >= SINSAMPLECOUNT) || (xi < 0.0))
		return 0.0;
	return sin_samples[xi];
}

float Utils::sinn(float x)
{
	return (sinf(x)+1)/2.0;
}

float Utils::cosf(float x)
{
	return sinf(x-0.25);
}

float Utils::cosn(float x)
{
	return (cosf(x)+1)/2.0;
}
